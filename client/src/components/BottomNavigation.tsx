  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 bg-slate-custom z-50 transition-transform duration-300 ${
      ...
      <div className="relative">
        {/* Curved cutout for betslip circle */}
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-4 z-10">
          <div className="w-20 h-8 bg-background rounded-t-full border-l border-r border-t border-gray-700"></div>
        </div>
        ...
        <div className="border-t border-gray-700 pt-2">
          <div className="grid grid-cols-5 max-w-md mx-auto">
            {navItems.map((item, index) => (
              ...
              >
                {index === 2 ? (
                  // Special circular design for betslip button that sits in the cutout
                  <div className="flex flex-col items-center -mt-8">
                    <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center mb-2 relative shadow-xl border-4 border-slate-custom">
                      <i className={`${item.icon} text-white text-xl`}></i>
                      {betslipCount > 0 && (
                        <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-6 h-6 rounded-full flex items-center justify-center font-bold border-2 border-slate-custom">
                          {betslipCount}
                        </span>
                      )}
                    </div>
                  ...
                  <>
                    ...
                    <span className="text-xs leading-tight text-center">{item.label}</span>
                    {item.count && item.count > 0 && index !== 2 && (
